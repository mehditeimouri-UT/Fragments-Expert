...
switch DecisionModel
    case {'Decision Tree','Random Forest','Ensemble kNN','Naive Bayes','Linear Discriminant Analysis (LDA)','Neural Network'}
...
switch DecisionModel
    ...
    case 'SVM'
        ...
        [DM,DM_CL,Pc,ConfusionMatrix,Pc_Train,ConfusionMatrix_Train,Pc_j,ConfusionMatrix_j,Pc_Train_j,ConfusionMatrix_Train_j,stopbar] = ...
            Build_MultiClassSVM_FFC(Dataset,ClassLabels_FFC,FeatureLabels_FFC,Weighting_Method,TIndex,VIndex,KernelFunction,PolynomialOrder,BoxConstraint,KernelScale,1);
        if stopbar
            ErrorMsg = 'Process is aborted by user.';
            return;
        end
