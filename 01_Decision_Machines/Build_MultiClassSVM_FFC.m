function [SVMModel_j,SVMModel_CL_j,Pc,ConfusionMatrix,Pc_Train,ConfusionMatrix_Train,Pc_j,ConfusionMatrix_j,Pc_Train_j,ConfusionMatrix_Train_j,stopbar] = ...
    Build_MultiClassSVM_FFC(Dataset,ClassLabels,FeatureLabels,Weights,Weighting_Method,TIndex,VIndex,KernelFunction,PolynomialOrder,BoxConstraint,KernelScale,prg_idx)
...
for j=1:M
    ...
    Weightsj = Assign_Weights_FFC(Datasetj(:,end-1),ClassLabelsj,Weighting_Method);
    
    % Train binary SVM
    [SVMModel_j{j},SVMModel_CL_j{j},Pc_j{j},ConfusionMatrix_j{j},Pc_Train_j{j},ConfusionMatrix_Train_j{j}] = ...
        Build_BinarySVM_FFC(Datasetj,ClassLabelsj,FeatureLabels,Weightsj,TIndex,VIndex,KernelFunction,PolynomialOrder,BoxConstraint,KernelScale);
    
    % Progress
    stopbar = progressbar_FFC(prg_idx,j/M);
    if stopbar
        SVMModel_j = [];
        SVMModel_CL_j = [];
        Pc = [];
...
[~,Pc_Train,ConfusionMatrix_Train,~,~] = Test_MultiClassSVM_FFC(SVMModel_j,Dataset,TIndex,ClassLabels,ClassLabels,FeatureLabels,FeatureLabels,Weights);
[~,Pc,ConfusionMatrix,~,~] = Test_MultiClassSVM_FFC(SVMModel_j,Dataset,VIndex,ClassLabels,ClassLabels,FeatureLabels,FeatureLabels,Weights);
