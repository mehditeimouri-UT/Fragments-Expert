function [Tree,Tree_CL,Pc,ConfusionMatrix,Pc_Train,ConfusionMatrix_Train,stopbar] = Build_DecisionTree_FFC(Dataset,ClassLabels,FeatureLabels,Weights,TIndex,VIndex,MinLeafSize,prg_idx)
...
for level = levels
    ...
    [~,Pcs(cnt)] = ConfusionMatrix_FFC(Validation(:,end-1),label_V,ClassLabels,ClassLabels,Validation_Weights);

    % progress indication
    stopbar = progressbar_FFC(prg_idx,cnt/length(levels));
    if stopbar
        Tree = [];
        Tree_CL = [];
        Pc = [];
...
[ConfusionMatrix_Train,Pc_Train] = ConfusionMatrix_FFC(Train(:,end-1),label_T,ClassLabels,ClassLabels,Train_Weights);

label_V = predict(Tree,Validation(:,1:end-2));
ConfusionMatrix = ConfusionMatrix_FFC(Validation(:,end-1),label_V,ClassLabels,ClassLabels,Validation_Weights);
