function [LDA_CL,Pc,ConfusionMatrix,Pc_Train,ConfusionMatrix_Train] = Build_LDA_FFC(Dataset,ClassLabels,FeatureLabels,Weights,TIndex,VIndex)

% This function takes a dataset and trains a linear discriminant analysis (LDA) classifier. 
%`
...
%
% Outputs:
%   LDA_CL: Linear discriminant analysis classifier with string class labels taken from ClassLabels
...
%
% Revisions:
% 2020-Mar-19   function was created
...
%% Train Naive Bayes
LDA_CL = fitcdiscr(Train(:,1:end-2),ClassLabels(Train(:,end-1)),...
    'PredictorNames',FeatureLabels,'Weights',Train_Weights,'DiscrimType','pseudoLinear');

%% Evaluate the performance of Naive Bayes Using Validation Set
[~,Pc_Train,ConfusionMatrix_Train,~,~] = Test_LDA_FFC(LDA_CL,Dataset,TIndex,ClassLabels,ClassLabels,FeatureLabels,FeatureLabels,Weights);
[~,Pc,ConfusionMatrix,~,~] = Test_LDA_FFC(LDA_CL,Dataset,VIndex,ClassLabels,ClassLabels,FeatureLabels,FeatureLabels,Weights);
