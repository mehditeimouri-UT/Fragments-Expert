function ydata = tsne_FFC(X, labels, no_dims, initial_dims, perplexity, max_iter)
...
GUI_MainEditBox_Update_FFC(false,'Preprocessing data using PCA...');
pause(0.01);
...
progressbar_FFC('Computing P-values ...');
for i=1:n
    
    if ~rem(i, 500)
        GUI_MainEditBox_Update_FFC(false,['Computed P-values ' num2str(i) ' of ' num2str(n) ' datapoints...']);
        pause(0.01);
    end
...
for iter=1:max_iter
    ...
    stopbar = progressbar_FFC(1,iter/max_iter);
    if stopbar
        if isempty(h)
